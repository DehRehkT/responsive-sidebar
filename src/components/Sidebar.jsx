...
import appLogo from '../assets/lumotechLogo.svg';
        ...
        <aside className={`h-screen flex transition-all ${expanded ? "w-[250px]" : "w-[80px]"}`}>
            <nav className="h-full flex flex-col bg-white border-r shadow-sm">
                <div className="p-4 pb-2 flex justify-between items-center">
                    ...
                    {
                        expanded && 
                        <div className="flex flex-row bg-gray-700 rounded-tl-2xl rounded-br-2xl py-2 px-3">
                            <span className="text-xl font-semibold text-white">Heat Staking</span>                   
                        </div>
                    }
...
export function SidebarItem({ icon, text, active, alert }) {
    ...
    return (
        <li className={`flex relative items-center py-2 px-3 my-1 font-medium rounded-md cursor-pointer transition-colors group  
            ${
        ...
        `}>
            {icon}
            { expanded && <span className={`overflow-hidden transition-all ${expanded ? "w-48 ml-3" : "w-0"}`}>{text}</span> }            
            {alert && <div className={`absolute right-2 w-2 h-2 rounded bg-indigo-400 ${expanded ? "": "left-8 top-2"}`}></div>}

            {
                !expanded && 
                    <div 
                        className={`absolute left-full rounded-md px-2 w-36 py-1 ml-6 bg-indigo-100 text-indigo-800 text-sm 
